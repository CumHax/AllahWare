...
import allah.owns.me.allahware.util.Pair;
		...
		if (mc.player != null || mc.world != null) {
			...
			for (BlockPos pos : spheres) {
				...
				if (checkDual(second_pos, air_orient)) {
					...
					if(safe_sides == 8) {
						holes.add(new Pair<BlockPos, Boolean>(pos, true));
						if (low_ceiling_hole) holes.add(new Pair<BlockPos, Boolean>(second_pos, true));
					}
					else {
						holes.add(new Pair<BlockPos, Boolean>(pos, false));
						if (low_ceiling_hole) holes.add(new Pair<BlockPos, Boolean>(second_pos, false));
					}
