        ...
        for (Map.Entry<Integer, ItemStack> armor_slot : get_armor().entrySet()) {
            ...
            if (mc.player.inventoryContainer.getInventory().get(5)== stack) {
                ...
                if (percent < 30 && !already_warned_helmet) {
                    if (Client.get_setting_manager().get_setting_with_tag("HUD", "notificationarmor").get_value(true)) NotificationUtil.send_notification(new Notification(notification + (int) percent + "%", 214, 38, 26));
