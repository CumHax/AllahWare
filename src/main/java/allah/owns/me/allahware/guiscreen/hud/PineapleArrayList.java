...
public class PineapleArrayList extends Pinnable {
	...
	public void render() {
		...
		List<Hack> pretty_modules = Client.get_hack_manager().get_array_active_hacks().stream()
			.sorted(Comparator.comparing(modules -> get(modules.array_detail() == null ? modules.get_tag() : modules.get_tag() + Client.g + " [" + Client.r + modules.array_detail() + Client.g + "]" + Client.r, "width")))
			.collect(Collectors.toList());

		int count = 0;

		if (Client.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top R") || Client.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top L") ) {
		...
		for (Hack modules : pretty_modules) {
			...
			if (flag) {
				String module_name = (
					...
					modules.get_tag() + Client.g + " [" + Client.r + modules.array_detail() + Client.g + "]" + Client.r
				);

				if (Client.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Free")) {
					...
					if (Client.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top R")) {
					...
					if (Client.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top L")) {
						mc.fontRenderer.drawStringWithShadow(module_name, 2, 3 + count * 10, new Draw.TravisColor(nl_r,nl_g,nl_b,nl_a).hex());
						count++;
					}
					if (Client.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Bottom R")) {
					...
					if (Client.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Bottom L")) {
